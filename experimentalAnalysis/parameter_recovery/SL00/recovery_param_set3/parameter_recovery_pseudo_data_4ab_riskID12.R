###############################################################################################
##
## 2-safe 2-risky bandit with exp condition 2 setting (riskID12)
...
# path -- drop box
dropbox_path <- "~/Dropbox/wataru/papers/RiskySocialLearning/experiment/parameter_recovery/SL00/recovery_param_set3/"
...
# -- Simulation
pseudoData_4ab_riskID12 = list()
conditionList = c('Group')


s_time = Sys.time()
for(condition in conditionList) {
	pseudoData_4ab_riskID12[[paste("n=", groupSize)]][[paste("condition=", condition)]] <- foreach(rep = 1:repetition, .combine=rbind) %dopar% {
		## Initial settings
...
# -- saving the data --
pseudoData_4ab_riskID12_data <- pseudoData_4ab_riskID12[[paste("n=", groupSize)]][[paste("condition=", "Group")]] %>% data.frame()

# pseudoData_4ab_riskID12_data$net_beta <- ( pseudoData_4ab_riskID12_data$log_beta + pseudoData_4ab_riskID12_data$epsilon * (pseudoData_4ab_riskID12_data$trial/horizon) ) %>% exp()

# pseudoData_4ab_riskID12_data$net_sigma <- ( pseudoData_4ab_riskID12_data$soc0 + pseudoData_4ab_riskID12_data$soc_slope *(pseudoData_4ab_riskID12_data$trial/horizon) ) %>% convert_alphaRaw_to_alpha


true_global_parameters_4ab_riskID12 <- data.frame(
	mu_alpha=mu_alpha
	, mu_beta=mu_beta
...
true_individual_parameters_4ab_riskID12 <- pseudoData_4ab_riskID12_data %>%
	group_by(indiv) %>%
...
# --- save files ---
write.csv(pseudoData_4ab_riskID12_data, paste0(dropbox_path, "pseudoData_4ab_riskID12_data.csv"), row.names=FALSE)
write.csv(true_global_parameters_4ab_riskID12, paste0(dropbox_path, "true_global_parameters_4ab_riskID12.csv"), row.names=FALSE)
write.csv(true_individual_parameters_4ab_riskID12, paste0(dropbox_path, "true_individual_parameters_4ab_riskID12.csv"), row.names=FALSE)
