// SL11: without temporal dynamics in soc_weight and temperature
...
functions {
  real net_choice_prob_lpmf(int Y, real logit_soc0, real logit_soc_slope, int trial, vector q, vector q_f, vector f) {
    if(trial == 1 || sum(f)==3e-10) {
      ...
      return log_sum_exp( log(1- 1/(1+exp(-(logit_soc0+(logit_soc_slope*trial/70)))) ) + categorical_lpmf(Y | exp(q)),
                          log( 1/(1+exp(-(logit_soc0+(logit_soc_slope*trial/70)))) ) + categorical_lpmf(Y | exp(q_f))
                        );
    }
  }
}
...
data {
    ...
    int<lower = 1> Ncon;  // number of experimental conditions
    int<lower = 1> condition[Nsub];
}
...
parameters {
    ...
    vector[Ncon] mu_soc_slope;
    vector[Ncon] mu_epsilon;

    vector<lower=0>[Ncon] s_alpha;
    vector<lower=0>[Ncon] s_beta;
    vector<lower=0>[Ncon] s_theta;
    vector<lower=0>[Ncon] s_soc0;
    vector<lower=0>[Ncon] s_soc_slope;
    vector<lower=0>[Ncon] s_epsilon;
    ...
    matrix[6,Nsub] z_ID;               //Matrix of uncorrelated z - values
    vector<lower=0>[6] sigma_ID;       //SD of parameters among individuals
    cholesky_factor_corr[6] L_Rho_ID;    // This is the Cholesky factor:
    //if you multiply this matrix and it's transpose you get correlation matrix
}

transformed parameters {
  matrix[Nsub,6] v_ID; // Matrix of varying effects for each individual for alpha, beta
  ...
  for(i in 1:Nsub) {
    logit_alpha[i] = mu_alpha[condition[i]] + s_alpha[condition[i]] * v_ID[i, 1];
    log_beta[i] = mu_beta[condition[i]] + s_beta[condition[i]] * v_ID[i, 2];
    epsilon[i] = mu_epsilon[condition[i]] + s_epsilon[condition[i]] * v_ID[i, 3];
    logit_soc0[i] = mu_soc0[condition[i]] + s_soc0[condition[i]] * v_ID[i, 4]; // logit_soc0 is translated into soc in the final function
    logit_soc_slope[i] = mu_soc_slope[condition[i]] + s_soc_slope[condition[i]] * v_ID[i, 5];
    theta[i] = mu_theta[condition[i]] + s_theta[condition[i]] * v_ID[i, 6];
  ...
  for(idx in 1:All) {
    ...
    if(trial[idx] == 1) {
      for(c in 1:Ncue) {
        Q[sub[idx]][c, trial[idx]] = 0;//1e-10; // Q[t==1] = 1e-10
      }
    }
...
model {
  ...
  for(idx in 1:All) {
    if(Y[idx] > 0) {
      target +=
        net_choice_prob_lpmf(Y[idx] | logit_soc0[sub[idx]], logit_soc_slope[sub[idx]], trial[idx], q[sub[idx]][,trial[idx]], q_f[sub[idx]][,trial[idx]], f[sub[idx]][,trial[idx]] );
    }
  }
}
...
generated quantities {
  ...
  matrix[6,6] Rho_ID; // the correlation matrix
