...
sigmaList = c(0, 0.1, 0.25, 0.5, 1, 2)
thetaList = c(0.5, 1, 2)
...
meanPayoffByRandomChoice = mean(c(mu_sure, mu_risky))

# -- Initial Settings --
outcomeBonus_riskPrem150_Gaussian_indivDiff = list()
	...
	for (sigma in sigmaList) {
		...
		for (alpha in alphaList) {
			for (invTemperature in invTemperatureList) {
				outcomeBonus_riskPrem150_Gaussian_indivDiff[[paste("n=", groupSize)]][[paste("alpha=", alpha)]][[paste("invTemperature=", invTemperature)]][[paste("sigma=", sigma)]][[paste("theta=", theta)]] <- foreach(rep = 1:repetition, .combine=rbind) %dopar% {
					## Initial settings
					...
					if (model != 'Outcome-Bonus') {
						...
						thisSigma = sigma + variationSigmaRaw * rt(groupSize, df = 14, ncp = 0)
					}
					...
					for(t in 1:horizon){
						...
						updatingPositions_outcomeBonus =
							which(socialFrequency[,t]>0) %>% rep(groupSize) +
							rep(0:(groupSize-1), each=length(which(socialFrequency[,t]>0)))*numOptions
						...
						if(t < horizon) {
							if(t == 1) {
								...
								dim(QQ) = c(numOptions, groupSize, horizon)
								Q = aperm(QQ, c(1,3,2))
							} else if (model != 'Outcome-Bonus' | sigma == 0) {
								...
								dim(QQ) = c(numOptions, groupSize, horizon)
								Q = aperm(QQ, c(1,3,2))
							} else { ## OUTCOME BONUS MODEL
					            QQ = aperm(Q, c(1,3,2))
					            dim(QQ) = c(numOptions*groupSize, horizon)
					            freqDepenMatrixDummy = freqDepenMatrix
					            dim(freqDepenMatrixDummy) = c(numOptions*groupSize,1)
					            # First, all values are copied from the past one
					            QQ[,t+1] = QQ[,t]
					            # Then, updating values for all options, including unchosen ones, according to social information
					            QQ[updatingPositions_outcomeBonus,t+1] = QQ[updatingPositions_outcomeBonus,t] + thisAlpha * (freqDepenMatrixDummy[updatingPositions_outcomeBonus,1]*meanPayoffByRandomChoice*thisSigma - QQ[updatingPositions_outcomeBonus,t])
					            # Finally, re-doing the update for the chosen option but now updating with both payoff and social information
					            QQ[updatingPositions,t+1] = QQ[updatingPositions,t] + thisAlpha * (payoffs[,t] + freqDepenMatrixDummy[updatingPositions,1]*meanPayoffByRandomChoice*thisSigma - QQ[updatingPositions,t])
					            dim(QQ) = c(numOptions, groupSize, horizon)
					            Q = aperm(QQ, c(1,3,2))
					        }
...
# -- saving the data --
outcomeBonus_riskPrem150_Gaussian_indivDiff_data = outcomeBonus_riskPrem150_Gaussian_indivDiff[[paste("n=",groupSizeList[1])]][[paste("alpha=",alphaList[1])]][[paste("invTemperature=",invTemperatureList[1])]][[paste("sigma=",sigmaList[1])]][[paste("theta=",thetaList[1])]] %>% data.frame()
			...
			for (alpha in alphaList) {
				for (invTemperature in invTemperatureList) {
					if(groupSize!=groupSizeList[1] | alpha!=alphaList[1] | invTemperature!=invTemperatureList[1]| sigma!=sigmaList[1] | theta!=thetaList[1]) {
						outcomeBonus_riskPrem150_Gaussian_indivDiff_data = outcomeBonus_riskPrem150_Gaussian_indivDiff_data %>% rbind(data.frame(outcomeBonus_riskPrem150_Gaussian_indivDiff[[paste("n=",groupSize)]][[paste("alpha=",alpha)]][[paste("invTemperature=",invTemperature)]][[paste("sigma=",sigma)]][[paste("theta=",theta)]]))
					}
				}
			}
		}
...
names(outcomeBonus_riskPrem150_Gaussian_indivDiff_data) = c('groupSize', 'learningRate', 'invTemperature', 'copyRate', 'conformityExp', 'proportionSafeChoice')


outcomeBonus_riskPrem150_Gaussian_indivDiff_summary <- outcomeBonus_riskPrem150_Gaussian_indivDiff_data %>%
...
outcomeBonus_riskPrem150_Gaussian_indivDiff_summary %>%
	ggplot() +
	...
	NULL -> outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_mean
outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_mean
...
outcomeBonus_riskPrem150_Gaussian_indivDiff_summary %>%
	...
	theme(axis.text.x = element_text(angle = 90))+
	facet_grid(copyRate ~ conformityExp)+
	NULL -> outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_sd
outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_sd

ggsave(file = "~/Dropbox/wataru/papers/RiskySocialLearning/riskPrem1.5_Gaussian/results/outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_mean.png", plot = outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_mean, dpi = 300, width = 9, height = 8)
ggsave(file = "~/Dropbox/wataru/papers/RiskySocialLearning/riskPrem1.5_Gaussian/results/outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_sd.png", plot = outcomeBonus_riskPrem150_Gaussian_indivDiff_plot_sd, dpi = 300, width = 9, height = 8)


write.csv(outcomeBonus_riskPrem150_Gaussian_indivDiff_data,
			"~/Dropbox/wataru/papers/RiskySocialLearning/riskPrem1.5_Gaussian/results/outcomeBonus_riskPrem150_Gaussian_indivDiff_data.csv",
			row.names=FALSE)

outcomeBonus_riskPrem150_Gaussian_indivDiff_data <- read_csv("~/Dropbox/wataru/papers/RiskySocialLearning/riskPrem1.5_Gaussian/results/outcomeBonus_riskPrem150_Gaussian_indivDiff_data.csv")
outcomeBonus_riskPrem150_Gaussian_indivDiff_data$learningRate = rep(alphaList, each = length(invTemperatureList)*repetition) %>% rep(length(groupSizeList))
